  ...
  if (loading) return (
    <div className="max-w-lg mx-auto px-4 py-20 text-center">
      <div className="relative w-36 h-36 mx-auto mb-6 animate-float">
        <img src="/IMG/empty-pozo.png" alt="Cargando POZOs" className="w-full h-full object-contain drop-shadow-lg" />
      </div>
      <p className="text-gray-500 animate-pulse">Cargando POZOs...</p>
    </div>
  );
    ...
    <div className="max-w-lg mx-auto px-4 py-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setSelectedPool(null)} className={`text-sm px-3 py-1.5 rounded-lg font-semibold transition-all ${selectedPool ? "btn-primary" : "hidden"}`}>
            ← Volver
          </button>
  ...
  return (
    <div className={`rounded-xl border p-3.5 space-y-2.5 transition-all duration-200 ${
      ...
      {pool.status === "active" && !event.resolved && !hasVoted && (
        <button onClick={onVote} className="btn-primary text-xs py-1.5 px-3">
          Confirmar evento
        </button>
      )}
